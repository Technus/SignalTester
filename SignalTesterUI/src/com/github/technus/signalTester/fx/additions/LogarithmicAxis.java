...
public class LogarithmicAxis extends ValueAxis<Number> {
    ...
    private DoubleProperty upperLogBound = new DoublePropertyBase(20000) {
        ...
        public String getName() {
            return "upperLogBound";
        }
    };
    public final double getUpperLogBound() { return upperLogBound.get(); }
    public final void setUpperLogBound(double value) { if(value>0)  upperLogBound.set(value); }
    public final DoubleProperty upperLogBoundProperty() { return upperLogBound; }

    /** The value for the lower bound of this axis, ie min value. This is automatically set if auto ranging is on. */
    private DoubleProperty lowerLogBound = new DoublePropertyBase(20) {
        @Override protected void invalidated() {
            if(!isAutoRanging()) {
                invalidateRange();
                requestAxisLayout();
        ...
        @Override
        public String getName() {
            return "lowerLogBound";
        }
    };
    public final double getLowerLogBound() { return lowerLogBound.get(); }
    public final void setLowerLogBound(double value) { if(value>0) lowerLogBound.set(value); }
    public final DoubleProperty lowerLogBoundProperty() { return lowerLogBound; }

    /** Base of the logarithm, set to 1 for linear */
    ...
    public final boolean isUsingLogBase(){ return logBase.get()!=1 && logBase.get()>0; }
    public final boolean isUsingLinBase(){ return logBase.get()==1 || logBase.get()<=0; }
    ...
    private DoubleProperty linTickUnit = new StyleableDoubleProperty(5) {
        @Override protected void invalidated() {
            if(!isAutoRanging()) {
                invalidateRange();
                requestAxisLayout();
    ...
    /**  The value between each major tick mark in data units. This is automatically set if we are auto-ranging. */
    private DoubleProperty logTickUnit = new StyleableDoubleProperty(.1) {
        @Override protected void invalidated() {
            if(!isAutoRanging()) {
                invalidateRange();
                requestAxisLayout();
    ...
    @Override protected void setRange(Object range, boolean animate) {
        ...
        if(animate) {
            ...
            ReadOnlyDoubleWrapper scaleWrapper=new ReadOnlyDoubleWrapper(scaleProperty().asObject(),scaleProperty().getName(),scaleProperty().doubleValue());
            currentAnimationID = animator.animate(
                    new KeyFrame(Duration.ZERO,
                            new KeyValue(currentLowerBound, oldLowerBound),
                            new KeyValue(scaleWrapper, getScale())
                    ),
                    new KeyFrame(Duration.millis(700),
                            new KeyValue(currentLowerBound, lowerBound),
                            new KeyValue(scaleWrapper, scale)
                    )
            );
        } else {
    ...
    protected List<Number> calculateMinorTickMarks() {
        ...
        final double tickUnit = getLinTickUnit();
    ...
    private Dimension2D measureTickMarkSize(Number value, double rotation, String numFormatter) {
        ...
        StringConverter<Number> formatter = getTickLabelFormatter();
        if (formatter == null) formatter = defaultFormatter;
        if(formatter instanceof LogarithmicAxis.DefaultFormatter) {
        ...
        private static final CssMetaData<LogarithmicAxis,Number> TICK_UNIT =
                new CssMetaData<LogarithmicAxis,Number>("-fx-tick-unit",
                    ...
                    public boolean isSettable(LogarithmicAxis n) {
                        return n.linTickUnit == null || !n.linTickUnit.isBound();
                    }

                    @Override
                    public StyleableProperty<Number> getStyleableProperty(LogarithmicAxis n) {
                        return (StyleableProperty<Number>)(WritableValue<Number>)n.linTickUnitProperty();
                    }
                };
